...
class AccountsDB():
    ...
    def register_account(self, CurUser) -> bool: 
        try:
            response = self.DB.table('users').insert({"email":f"{CurUser['email']}","login":f"{CurUser['login']}","name":f"{CurUser['name']}","password":f"{CurUser['password']}"}).execute()
        except:
